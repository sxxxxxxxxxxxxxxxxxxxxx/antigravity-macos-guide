...
* ⬜ **如果是白色的**：那是新出的 **Antigravity 2.0**（新的项目/Agent 工作台）。
* ⬛ **如果是黑色的**：并且有完整编辑器、扩展市场和类似 VS Code 的左侧栏，那就是 **Antigravity IDE**。

---

## 三、必做：MacOS 网络环境配置（二选一）
...
3. **扫码验证核心操作步骤**：
   ...
   * **环境一致性（关键）**：手机和 Mac 必须保持网络环境一致。建议手机和 Mac 连接**同一个美国节点（同一个 US IP）**，确保底层网络环境干净一致。
   ...
   * 打开 Google App，点击搜索框右侧的智能镜头图标，对准 Mac 验证页面上的二维码扫描。
   ...
   * 首次扫码通常即可通过；完成后返回 Mac 上的 Antigravity，等待页面自动刷新，必要时重新发送一次测试消息。
   * ⚠️ **【严重风控警告】**：如果扫码后没有反应或失败，**千万不要连续扫码超过十几次**！频繁扫码极易触发 Google 的风控机制，导致账号被锁定大约**七天甚至一个月**。在这期间“绝对没救”，只能耐心等待风控解除。
6. **扫码后没有反应或反复出现二维码**：确认安卓手机的 Google App 已登录正确的 Google 账号、Google Play 商店能够正常打开，并检查手机代理是否确实生效。二维码可能自动刷新或过期，请扫描电脑页面当前显示的最新二维码，不要扫描旧截图。
7. **报错 400 / 500 处置**：若点击蓝色按钮后网页提示 400 或 500 错误，通常说明当前默认浏览器没有登录这个 Google 账号。请在 Mac 设置中将默认浏览器切换为您刚登录 Google 账号的那个浏览器，或在该默认浏览器中重新登录一次对应账号。
...
| `oauth2.googleapis.com/token`、`connectex`、连接超时 | 网络没真正走代理 | 异常 0.5 的 OAuth 网络失败 |
| `There was an error with your authentication` | 本地授权状态或 Token 状态异常 | 异常 0.5 的认证错误 |
| 点击登录后接近 1 分钟没反应 | 客户端请求超时，网络没接管 | 异常 1 |
...
* **对策**：请按顺序处理：
  ...
  2. 从头复盘第三部分的【网络环境深度配置】，确认 Tun 真正开启；如果使用 Antify，请确认应用规则、代理端口和代理类型都填对。
...
* **`EOF`**：通常是请求中途断开，优先更换稳定节点，并确认 Tun / Antify 已真正接管客户端流量。
